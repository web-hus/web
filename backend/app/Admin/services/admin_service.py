...
from ...models import user_model, order_model, booking_model, dish_model
...
class AdminService:
    ...
    def create_dish(db: Session, dish_data: DishCreate):
        """Create new dish"""
        try:
            # Kiểm tra dish_id đã tồn tại chưa
            existing_dish = db.query(dish_model.Dish).filter(
                dish_model.Dish.dish_id == dish_data.dish_id
            ).first()
            
            if existing_dish:
                raise HTTPException(
                    status_code=400, 
                    detail=f"Dish with ID {dish_data.dish_id} already exists"
                )
